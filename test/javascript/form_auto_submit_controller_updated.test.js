...
const FormAutoSubmitController = class {
    ...
    validateSingleField(field) {
        ...
        if (
            (fieldType === "email" || fieldName.includes("email")) &&
            value.trim().length > 0
        ) {
        ...
        if (fieldType === "password" && !fieldName.includes("confirmation")) {
            return !(
                value.length > 0 && value.length < this.minPasswordLengthValue
            );
        }

        // Validate password confirmation
        if (fieldName.includes("confirmation") && fieldType === "password") {
            const passwordField = this.formTarget.querySelector(
                'input[type="password"]:not([name*="confirmation"])',
            );
...
describe("FormAutoSubmitController (Updated)", () => {
    ...
    `;
        ...
        passwordConfirmInput = form.querySelector(
    ...
    test("validates password confirmation", () => {
        ...
        expect(controller.validateSingleField(passwordConfirmInput)).toBe(
            false,
        );
