...
describe('DismissibleController', () => {
  ...
  const DismissibleControllerClass = class {
    constructor(element, xmlrpcFn = mockXmlrpc) {
      this.element = element;
      this.xmlrpc = xmlrpcFn;
    ...
    dismiss() {
      ...
      // Call to server
      return this.xmlrpc(this.targetValue, 'dismiss', [this.idValue])
        .catch(error => {
  ...
  test('dismiss calls xmlrpc with correct parameters', async () => {
    ...
    expect(mockXmlrpc).toHaveBeenCalledWith('test_target', 'dismiss', ['123']);
  });
  
  test('dismiss handles errors gracefully', async () => {
    // Make xmlrpc fail
    mockXmlrpc.mockImplementationOnce(() => Promise.reject(new Error('Test error')));
