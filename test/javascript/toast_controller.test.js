...
const ToastControllerClass = class {
  ...
  showToasts() {
    ...
    const toastElements = this.element.querySelectorAll('.toast:not(.show)');
...
describe('ToastController', () => {
  ...
  test('hideToast removes show class and deletes toast after delay', () => {
    ...
    let timeoutCallback;
    global.setTimeout = jest.fn((fn, _) => {
      timeoutCallback = fn;
  ...
  test('createToast uses default title when none provided', () => {
    ...
    expect(toast.querySelector('.toast-header strong').textContent).toBe('Notification');
  });
