...
class CachePreloadJob < ApplicationJob
  ...
  def perform(sitemap_url: nil, max_urls: Integer(ENV.fetch('CACHE_PRELOAD_MAX_URLS', '1000')), rate_limit_ms: Integer(ENV.fetch('CACHE_PRELOAD_RATE_LIMIT_MS', '100')))
    base = base_url&.chomp('/')
    return unless base
    ...
    urls.each_with_index do |url, idx|
      ...
      sleep(rate_limit_ms.to_f / 1000.0) if idx < urls.size - 1
    end
  end
  ...
  def base_url
    ...
    cfg = Rails.application.config_for(:seo_config) rescue nil
