...
export default class extends ApplicationController {
    ...
    toggle(event) {
        ...
        this.stimulate("DrawerReflex#toggle", currentState)
            .then(() => {
                // Get HTML after reflex
                const afterHtml = this.element.outerHTML;
                console.log(
                    "Drawer toggle clicked - After Reflex HTML:",
                    afterHtml,
                );
                ...
                if (wholeDiff.hasDiff) {
                    console.log(
                        "HTML Differences in drawer:",
                        wholeDiff.message,
                    );
                    console.table(wholeDiff.attributeDiffs);
                    if (wholeDiff.contentDiff.hasChanges) {
                        console.log(
                            "Content differences at position",
                            wholeDiff.contentDiff.position,
                        );
                        console.log(
                            "Before:",
                            wholeDiff.contentDiff.beforeContext,
                        );
                        console.log(
                            "After:",
                            wholeDiff.contentDiff.afterContext,
                        );
                    }
                } else {
                ...
                if (updatedDrawerContent) {
                    ...
                    if (beforeContentHtml) {
                        ...
                        if (contentDiff.hasDiff) {
                            ...
                            if (contentDiff.contentDiff.hasChanges) {
                                ...
                                console.log(
                                    "Before:",
                                    contentDiff.contentDiff.beforeContext,
                                );
                                console.log(
                                    "After:",
                                    contentDiff.contentDiff.afterContext,
                                );
                            }
                        }
                    }
                    ...
                    // Check if emoji image tags exist
                    const updatedEmojiImgs =
                        updatedDrawerContent.querySelectorAll("img.emoji");
                    console.log(
                        `Found ${updatedEmojiImgs.length} emoji images after reflex`,
                    );
                    if (updatedEmojiImgs.length > 0) {
                        console.log(
                            "Sample emoji image after reflex:",
                            updatedEmojiImgs[0].outerHTML,
                        );

                        // Compare emoji images if they exist in both before and after
                        const emojiImgs =
                            drawerContent?.querySelectorAll("img.emoji");
                        if (
                            emojiImgs?.length > 0 &&
                            updatedEmojiImgs.length > 0
                        ) {
                            const emojiDiff = diffHtml(
                                emojiImgs[0].outerHTML,
                                updatedEmojiImgs[0].outerHTML,
                            );
                            if (emojiDiff.hasDiff) {
                                console.log(
                                    "Emoji image differences:",
                                    emojiDiff.message,
                                );
