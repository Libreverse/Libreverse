...
export default class extends Controller {
    ...
    connect() {
        ...
        if (!validateLiquidGlass(this.element)) {
            console.warn(
                "[GlassController] Glass validation failed, using CSS fallback",
            );
            this.setupFallback();
            return;
        }
    ...
    initializeGlass() {
        ...
        try {
            ...
            this.glassContainer = this.renderGlassComponent(
                navItems,
                containerOptions,
                renderOptions,
            );
    ...
    getNavItems() {
        ...
        const navItemsData =
            this.data.get("nav-items") || this.element.dataset.navItems;
        ...
        try {
            ...
            console.error(
                "[GlassController] Failed to parse nav items:",
                error,
            );
            return [];
        }
    ...
    renderGlassComponent(navItems, containerOptions, renderOptions) {
        ...
        if (componentType === "drawer") {
            return renderLiquidGlassDrawer(
                this.element,
                containerOptions,
                renderOptions,
            );
        } else if (componentType === "sidebar" && cornerRounding === "right") {
            return renderLiquidGlassSidebarRightRounded(
                this.element,
                navItems,
                containerOptions,
                renderOptions,
    ...
    postRenderSetup() {
        ...
        if (
            this.componentTypeValue === "nav" ||
            this.componentTypeValue === "sidebar"
        ) {
    ...
    cleanupGlass() {
        try {
            if (
                this.glassContainer &&
                typeof this.glassContainer.destroy === "function"
            ) {
    ...
    restoreOriginalContent() {
        ...
        const existingContent = this.element.querySelector(
            ".sidebar-contents, .nav-contents, .card-contents",
        );
        if (existingContent) {
