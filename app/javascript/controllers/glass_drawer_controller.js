...
export default class extends GlassController {
  ...
  connect() {
    super.connect()
    console.log('[GlassDrawerController] Connected', { element: this.element, expanded: this.expandedValue })
    ...
    document.addEventListener("drawer:toggle", this.boundDrawerEventHandler)
  ...
  disconnect() {
    ...
    document.removeEventListener("drawer:toggle", this.boundDrawerEventHandler)
    super.disconnect()
  }
  ...
  updateDrawerHeight() {
    ...
    const targetHeight = this.expandedValue ? `${this.expandedHeightValue}px` : `${this.heightValue}px`
    const contentHeight = this.expandedValue ? `${this.expandedHeightValue - this.heightValue}px` : '0px'
  ...
  updateAriaExpanded() {
    if (this.hasDrawerTarget) {
      this.drawerTarget.setAttribute('aria-expanded', this.expandedValue.toString())
    }
  }
  ...
  expandedValueChanged() {
    console.log(`[GlassDrawerController] Expanded state changed to: ${this.expandedValue}`)
    this.updateUI()

    // Dispatch events to notify other parts of the application.
    const eventName = this.expandedValue ? 'drawer:opened' : 'drawer:closed'
