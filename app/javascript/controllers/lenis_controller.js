...
export default class extends Controller {
        ...
        try {
            ...
            if (!contentElement) {
                this.log("init:warning:no_content_element", {
                    expected:
                        "Child element matching [data-lenis-content] or .lenis-content",
                    wrapperInnerHTMLPreview: this.element.innerHTML?.slice?.(
                        0,
                        300,
                    ),
                });
            ...
            requestAnimationFrame(() => {
                ...
                const shouldBeScrollable =
                    wrapperScrollHeight > wrapperClientHeight + 1;
                ...
                if (shouldBeScrollable && (this.lenis?.limit ?? 0) === 0) {
                    console.warn(
                        "[Lenis] Disabled: limit=0 even though wrapper has overflow. Falling back to native scroll.",
                        {
                            wrapperClientHeight,
                            wrapperScrollHeight,
                            wrapperOverflow: getComputedStyle(this.element)
                                .overflow,
                            wrapperOverflowY: getComputedStyle(this.element)
                                .overflowY,
                        },
                    );
                    this.disabled = true;
    ...
    destroyIfNeeded = (event) => {
        this.log("turbo:before:*", {
            type: event?.type,
            targetController: event?.target?.controller,
        });
        if (
            this.lenis &&
    ...
    setupEventListeners() {
        ...
        if (this.debug) {
            this.element.addEventListener("wheel", this.boundDebugWheel, {
                passive: true,
            });
            this.element.addEventListener(
                "touchmove",
                this.boundDebugTouchMove,
                { passive: true },
            );
        }
    }
    ...
    ensureContentElement() {
        let contentElement = this.element.querySelector(
            "[data-lenis-content], .lenis-content",
        );
