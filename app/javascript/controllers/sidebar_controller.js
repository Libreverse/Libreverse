...
export default class extends Controller {
    ...
    initializeLiquidGlass() {
        ...
        if (!navItemsData) {
            throw new Error(
                "nav-items data attribute is required for liquid glass sidebar",
            );
        }
        ...
        const navItems = rawNavItems.map((item) => {
            if (!item.icon) {
                throw new Error(
                    `Nav item missing required icon: ${JSON.stringify(item)}`,
                );
            }
        ...
        this.glassContainer = renderLiquidGlassSidebarRightRounded(
            nav,
            navItems,
            containerOptions,
            {
        ...
        console.log(
            "[SidebarController] Liquid glass initialized with",
            navItems.length,
            "items",
        );
    ...
    cleanupLiquidGlass() {
        try {
            ...
            console.error(
                "[SidebarController] Error cleaning up liquid glass:",
                error,
            );
        }
    }
