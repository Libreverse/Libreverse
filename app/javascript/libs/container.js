...
class Container {
        ...
        requestAnimationFrame(() => {
            ...
            if (newWidth !== this.width || newHeight !== this.height) {
                ...
                for (const child of this.children) {
                    if (
                        typeof child.isNestedGlass === "boolean" &&
                        child.isNestedGlass &&
                        child.gl_refs &&
                        child.gl_refs.gl
                    ) {
    ...
    capturePageSnapshot() {
        ...
        console.log(
            `Waiting containers: ${Container.waitingForSnapshot.length}`,
        );
        ...
        html2canvas(document.body, {
            ...
            .then((snapshot) => {
                console.log(
                    `Page snapshot captured: ${snapshot.width}x${snapshot.height}`,
                );
  ...
  `;
    ...
    `;
        ...
        const roundedCornersLoc = gl.getUniformLocation(
            program,
            "u_roundedCorners",
        );
        ...
        try {
            ...
            if (
                ...
                for (const loc of locations) {
                    ...
                    console.log(
                        `[Container Debug] Texture pixel sample ${loc.name} (${loc.x},${loc.y}):`,
                        {
                            r: pixels[0],
                            g: pixels[1],
                            b: pixels[2],
                ...
                console.warn(
                    "[Container Debug] Framebuffer not complete for texture reading",
                );
            }
            ...
            console.warn(
                "[Container Debug] Could not read texture pixels:",
                error,
            );
        }
    ...
    startRenderLoop() {
        const render = () => {
            ...
            if (this.isParallaxElement && this.parallaxSpeed !== 1.0) {
                ...
                if (Math.random() < 0.01) {
                    console.log(
                        "[Container Debug] Parallax element position correction:",
                        {
                            viewportY: position.y,
                            scrollY: scrollY,
        ...
        */
            ...
            if (Math.random() < 0.01) {
                ...
                const textureWidth =
                    this.gl_refs.textureWidth ||
                    Container.pageSnapshot?.width ||
                    1;
                const textureHeight =
                    this.gl_refs.textureHeight ||
                    Container.pageSnapshot?.height ||
                    1;
                const viewportHeight = window.innerHeight;
                const pageHeight = Math.max(
                    document.body.scrollHeight,
    ...
    static setupGlobalEventHandlers() {
        ...
        if (!Container._globalHandlersSetup) {
            ...
            document.addEventListener(
                "turbo:before-cache",
                Container._handleTurboBeforeCache,
            );
    ...
    static _recapturePageSnapshot() {
        ...
        if (now - Container.lastCaptureTime < Container.captureDebounceDelay) {
            console.log(
                "Skipping page recapture - too soon since last capture",
            );
            return;
        }
        ...
        html2canvas(document.body, {
            ...
            .then((snapshot) => {
                console.log(
                    `Page snapshot recaptured after resize: ${snapshot.width}x${snapshot.height}`,
                );
                Container.pageSnapshot = snapshot;
                Container.isCapturing = false;
                    ...
                    for (const instance of Container.instances) {
                        ...
                        for (const child of instance.children) {
                            if (
                                ...
                                console.log(
                                    `Updated nested button texture: ${containerCanvas.width}x${containerCanvas.height}`,
                                );
