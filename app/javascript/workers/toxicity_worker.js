...
self.onmessage = async (e) => {
  ...
  try {
    if (action === "init") {
      const { tokenizerFile, modelConfigFile, tokenizerConfigFile, onnxFile } = payload || {};
      try {
        // If we have a File, create a data/blob URL and store it for remapping
        if (onnxFile) {
          currentOnnxUrl = URL.createObjectURL(onnxFile);
        }
        const customModel = onnxFile ? { file: onnxFile, format: "onnx" } : null;
        const pipe = await loadToxicityPipeline(tokenizerFile || null, modelConfigFile || null, tokenizerConfigFile || null, customModel);
        if (!pipe) {
          postMessage({ type: "init-failed", error: "pipeline-null" });
          return;
        }
      ...
      if (!pipelineInstance) {
        postMessage({ type: "result-error", id, error: "pipeline-not-initialized" });
        return;
      }
      try {
