...
class MetaverseExperienceImportService
  ...
  def extract_description(indexed_content)
    ...
    if indexed_content.coordinates.present? && indexed_content.coordinates['x'].present?
      description += " at coordinates (#{indexed_content.coordinates['x']}, #{indexed_content.coordinates['y']})"
    end
  ...
  def extract_coordinates_json(indexed_content)
    return nil unless indexed_content.coordinates.present?
