...
sub is_git_repo {
  return
    system('git', '-C', $repo_root, 'rev-parse', '--is-inside-work-tree',
    '>/dev/null', '2>&1') == 0;
}
...
for my $e (@entries) {
  ...
  my $base = $new_name;
  my ($stem, $ext) = $new_name =~ /^(.*?)(\.[^.]+)?$/;
  $ext //= '';
  my $i = 1;

  while (exists $new_to_old{$new_rel}
    || -e File::Spec->catfile($repo_root, $new_rel)) {
...
print($do_apply ? "Renaming" : "Would rename"), " the following files (",
  scalar(keys %map), "):\n\n";
...
for my $old (sort keys %map) {
  ...
  if ($use_git) {
    ...
    if   ($ret == 0) { $ok++ }
    else             { warn "git mv failed: $old -> $new\n"; $fail++ }
  } else {
    ...
    if (rename $old_abs, $new_abs) { $ok++ }
    else { warn "rename failed: $old -> $new ($!)\n"; $fail++ }
  }
}
...
my @md = grep {/\.md$/i}
  map { File::Spec->catfile($docs_dir, $_) } grep { !/^\./ } readdir($dh);
closedir $dh;
