...
for my $arg (@ARGV) {
  my ($k, $v) = split /=/, $arg, 2;
  die "Bad override '$arg' (need KEY=VAL)" unless defined $v;
  $defaults{$k} = $v;
}
...
for my $k (sort keys %defaults) {
  next
    if $defaults{$k} eq '' && $k =~ /PASSWORD|USERNAME/;  # skip empty auth vars
  push @env_pairs, '--env', "$k=$defaults{$k}";
}

my @cmd = (
  'perl',   'scripts/run_waf_det.pl', '--image',  'libreverse:waf-det',
  '--port', '3000',                   '--logdir', './waf_logs',
  @env_pairs
);
print "+ @cmd\n";
