...
unless (command_exists('bun')) {
  ...
  my $download_cmd =
"curl -fsSL --tlsv1.2 --proto '=https' --max-time 30 --retry 3 -o '$temp_script' 'https://bun.sh/install'";
...
# Copy local gh-install script to temp location using File::Copy for portability
  unless (copy($local_script, $install_script)) {
    die
"Failed to copy local gh-install script from $local_script to $install_script: $!";
  }
  ...
  if ($is_windows) {
    ...
    $ENV{PATH} = "$cargo_bin:$ENV{PATH}" unless $ENV{PATH} =~ /\Q$cargo_bin\E/;
    $ENV{PATH} = "$homebrew_intel:$ENV{PATH}"
      unless $ENV{PATH} =~ /\Q$homebrew_intel\E/;
    $ENV{PATH} = "$homebrew_arm:$ENV{PATH}"
      unless $ENV{PATH} =~ /\Q$homebrew_arm\E/;
  }
...
if ($codeql_exit_code == 0) {
  ...
  print
"  Warning: CodeQL CLI installation failed. Security analysis may not work.\n";
