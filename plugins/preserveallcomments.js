  ...
  return {
    ...
    generateBundle(options, bundle) {
      for (const fileName in bundle) {
        ...
        if (item.type === 'chunk') { // For JS chunks
          const comments = [];
          try {
            ...
            console.warn(`Failed to parse ${fileName} for comments: ${error.message}`);
            continue;
          }
          ...
          for (const comment of comments) {
            ...
            newCode = newCode.slice(0, insertPos) + '!' + newCode.slice(insertPos);
          }
          item.code = newCode;
        }
      }
